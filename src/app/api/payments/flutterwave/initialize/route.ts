...
export async function POST(req: NextRequest) {
  ...
  const txRef = `${order.code}-${Date.now().toString(36).toUpperCase()}`;
  ...
  try {
    const init = await flutterwaveInitialize({
      ...
      txRef,
      redirectUrl: `${siteUrl}/checkout/callback?provider=flutterwave`,
    ...
    await prisma.$transaction([
      prisma.payment.create({
        data: { orderId: order.id, provider: "FLUTTERWAVE", reference: txRef, amountKobo: order.totalKobo, status: "INITIATED" },
      }),
      prisma.order.update({ where: { id: order.id }, data: { paymentMethod: "FLUTTERWAVE" } }),
    ]);
